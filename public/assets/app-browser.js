(() => {
  ...
  const defaultState = {
    session: { role: 'teacher', authMode: 'demo', userName: 'Alan Teacher', email: 'teacher@questclass.app', uid: null },
  ...
  function renderAuthPanel(session = {}, firebaseEnabled = false) { const signedIn = session.authMode === 'firebase' && session.uid; const roleBadge = session.role ? `<span class="pill">角色：${escapeHtml(session.role)}</span>` : ''; return `<div class="auth-shell"><div><span class="eyebrow">Auth</span><h3>${signedIn ? `已登入：${escapeHtml(session.userName || session.email || 'QuestClass User')}` : '登入 QuestClass'}</h3><p class="muted">${firebaseEnabled ? '支援 Google OAuth。' : '目前是 demo 模式。'} ${roleBadge}</p></div><div class="inline-actions auth-actions">${firebaseEnabled ? `${signedIn ? '<button class="button button-ghost" type="button" data-auth-action="logout">Logout</button>' : ''}<button class="button button-primary" type="button" data-auth-action="google">${signedIn ? '切換 Google 帳號' : '使用 Google 登入'}</button>` : ''}</div>${firebaseEnabled && !signedIn ? `<form class="auth-form" data-auth-form><input name="email" type="email" placeholder="teacher@questclass.app" /><input name="password" type="password" placeholder="Password" /><button class="button button-ghost" type="submit">Email 登入</button></form>` : ''}</div>`; }
  ...
  function setShell() { qsa('[data-nav]').forEach((n) => n.innerHTML = renderNav(page)); qsa('[data-brand]').forEach((n) => n.textContent = `${demoData.brand.name} ${demoData.brand.version}`); qsa('[data-promise]').forEach((n) => n.textContent = demoData.brand.promise); }
  ...
  function applyFirebaseUser(user) { state.session.authMode = 'firebase'; state.session.role = user.role || state.session.role; state.session.userName = user.name || state.session.userName; state.session.email = user.email || state.session.email; state.session.uid = user.uid || state.session.uid; saveState(state); }
  function clearFirebaseSession() { state.session.authMode = 'demo'; state.session.role = 'teacher'; state.session.userName = 'Alan Teacher'; state.session.email = 'teacher@questclass.app'; state.session.uid = null; saveState(state); }
  ...
  function wireAuthUi() { const firebaseEnabled = !!window.QuestClassFirebase?.enabled?.(); qsa('[data-auth-panel]').forEach((node) => { node.innerHTML = renderAuthPanel(state.session, firebaseEnabled); const authForm = qs('[data-auth-form]', node); if (authForm) authForm.onsubmit = async (e) => { e.preventDefault(); const fd = new FormData(authForm); const result = await window.QuestClassFirebase.signIn(fd.get('email'), fd.get('password')); if (!result.ok) return showToast(result.error || 'Email 登入失敗'); applyFirebaseUser(result.user); firebaseProfile = result.user.profile || firebaseProfile; await refreshAdminUsers(); renderAll(); enforcePageGuard(); showToast('已登入'); }; }); qsa('[data-auth-action="google"]').forEach((b) => b.onclick = handleGoogleLogin); qsa('[data-auth-action="logout"]').forEach((b) => b.onclick = handleLogout); }
