(() => {
  ...
  function renderDebugPanel() { qsa('[data-debug-panel]').forEach((node) => { if (state.session.authMode !== 'firebase' || !state.session.uid) { node.innerHTML = '<div class="panel-header"><div><span class="eyebrow">Debug</span><h3>登入除錯資訊</h3></div></div><code>尚未登入 Firebase</code>'; return; } node.innerHTML = `<div class="panel-header"><div><span class="eyebrow">Debug</span><h3>登入除錯資訊</h3></div></div><code>email=${escapeHtml(state.session.email || '')}\nuid=${escapeHtml(state.session.uid || '')}\nrole=${escapeHtml(state.session.role || '')}\nprofileRole=${escapeHtml(state.session.profileRole || '')}\nderivedRole=${escapeHtml(state.session.derivedRole || '')}</code>`; }); }
  function renderAll() { setShell(); wireSettings(); wireFirebaseStatus(); renderDebugPanel(); renderLanding(); renderTeacher(); renderStudent(); renderChatPage(); renderAnalytics(); wireAuthUi(); wireProfileUi(); }
