(() => {
  ...
  function renderAuthPanel(session = {}, firebaseEnabled = false) { const signedIn = session.authMode === 'firebase' && session.uid; if (!firebaseEnabled) return ''; const avatar = session.photoURL ? `<img class="account-avatar" src="${escapeHtml(session.photoURL)}" alt="avatar" />` : '<span class="account-avatar account-avatar-fallback">G</span>'; const label = signedIn ? `${avatar}<span>${escapeHtml(session.userName || session.email || 'Google 帳號')}</span>` : '使用 Google 登入'; return `<div class="inline-actions auth-actions">${signedIn ? '<button class="button button-ghost" type="button" data-auth-action="logout">Logout</button>' : ''}<button class="button button-primary account-button" type="button" data-auth-action="google">${label}</button></div>`; }
