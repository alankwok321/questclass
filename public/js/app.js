const STORAGE_KEY = 'questclass_state_v3';
...
const defaultState = {
  ...
  demoUsers: {
    teacher: [
      { id: 't-alan', name: 'Alan Teacher', email: 'teacher@questclass.app' },
      { id: 't-amy', name: 'Amy Teacher', email: 'amy@questclass.app' }
    ],
    student: [
      { id: 'ada', name: 'Ada', email: 'ada@student.questclass.app' },
      { id: 'mia', name: 'Mia', email: 'mia@student.questclass.app' },
      { id: 'leo', name: 'Leo', email: 'leo@student.questclass.app' }
    ]
  },
  students: [
...
function renderAuth() {
  ...
  authLoginPanel.style.display = state.session.authTab === 'login' ? '' : 'none';
  roleTeacherBtn.classList.toggle('active', state.session.role === 'teacher');
  roleStudentBtn.classList.toggle('active', state.session.role === 'student');
  demoUserList.innerHTML = state.demoUsers[state.session.role].map(user => `<button class="stack-card ${user.id===state.session.userId?'active':''}" onclick="selectDemoUser('${user.id}')"><strong>${user.name}</strong><span>${user.email}</span></button>`).join('');
}
...
function completeQuest(index) { if (!state.dailyQuests[index] || state.dailyQuests[index].done) return; state.dailyQuests[index].done = true; const student = currentStudent(); student.xp += 90; renderSidebar(); updateHero(); saveState(); showToast('任務完成，XP +90'); }
